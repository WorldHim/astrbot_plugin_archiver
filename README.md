...
- **入典**：回复一条消息并发送 `/入典`，将该消息的发送人与内容（文本、图片，其他类型以占位符记录）存档进当前会话的典库。**归属默认为回复消息的发送者；回复的内容为聊天记录（合并转发）时，归属取其中最后一条消息的发送者（以 QQ 号保存，避免群昵称变化影响）**，且**不展开为文本，而是以聊天记录（结构化子消息：发送人/文本/图片）原样保存**；拉取失败时回退为 `[转发消息]` 占位符。
