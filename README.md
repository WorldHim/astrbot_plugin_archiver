...
- **来点典**：发送 `/来点典`，从当前会话典库中随机抽取一条典，以**聊天记录（合并转发消息）**的形式发回：普通典以「发送人 + 内容」节点发送；**聊天记录典保持聊天记录形态逐条发出，并在最后附一条收录信息**（收录人/收录时间）。**支持抽取指定人的典**：`/来点典 @某人`（以 QQ 号精确匹配，`@Bot` 唤醒自身会被排除）或 `/来点典 昵称`（昵称完整或包含匹配）；归属以 QQ 号保存，不受群昵称变化影响。
